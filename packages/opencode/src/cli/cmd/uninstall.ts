...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall bob and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g @hiai-bob/cli",
      pnpm: "pnpm uninstall -g @hiai-bob/cli",
      bun: "bun remove -g @hiai-bob/cli",
      // TODO(bob): uncomment when published to these channels
      // brew: "brew uninstall bob",
      // choco: "choco uninstall bob",
      // scoop: "scoop uninstall bob",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Bob!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# bob") || content.includes(".bob/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".bob/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".bob/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".bob"))
    ) {
      continue
    }
