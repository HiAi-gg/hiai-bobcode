...
export const ActorTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    const taskSvc = yield* TaskRegistry.Service
    ...
    return Effect.fn("ActorTool.init")(function* () {
      ...
      // Include "all"-mode agents too: they're picker-selectable AND delegatable
      // (e.g. Bob's Strategist appears in the agent picker yet is still spawnable).
      const spawnable = allAgents.filter((a) => (a.mode === "subagent" || a.mode === "all") && !a.hidden)
      ...
      const run = Effect.fn("ActorTool.execute")(function* (input: z.infer<typeof parameters>, ctx: Tool.Context) {
        ...
        if (op.task_id) {
          if (!TaskID.safeParse(op.task_id).success) {
            ...
            const existing = yield* taskSvc.get({ session_id: ctx.sessionID, id: op.task_id })
            if (!existing) {
              effectiveTaskId = undefined
