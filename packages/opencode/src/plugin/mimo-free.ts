...
// DISABLED for hiai-bob: upstream MiMo free API is not connected.
...
export async function MimoFreeAuthPlugin(_input: PluginInput): Promise<Hooks> {
  return {
    config: async (input) => {
      ...
      // No providers are force-disabled. Upstream hiai disabled "opencode" and
      // "opencode-go"; we leave ALL providers connectable via /connect. (Users
      // can still disable any provider explicitly through `disabled_providers`
      // in their own config.)
    },
  }
}
