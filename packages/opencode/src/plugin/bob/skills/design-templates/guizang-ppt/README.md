...
一个 [Agent Skills](https://agentskills.io/) 技能,用于生成**单文件 HTML 横向翻页 PPT**,视觉基调是"**电子杂志 × 电子墨水**"——像 *Monocle* 贴上了代码的样子。
...
> 帮我安装 `guizang-ppt-skill` 这个 agent skill。请按下面步骤做:
>
> 1. 确保 `~/.agent/skills/` 目录存在(不存在就创建)
> 2. 执行 `git clone https://github.com/op7418/guizang-ppt-skill.git ~/.agent/skills/magazine-web-ppt`
> 3. 验证:`ls ~/.agent/skills/magazine-web-ppt/` 应该看到 `SKILL.md`、`assets/`、`references/` 三项
> 4. 告诉我安装好了,之后我说"做一份杂志风 PPT"之类的话就会触发这个 skill

把这段话复制粘贴给你的 agent,它会自动完成安装。
...
装好后,你的 agent 会在对话里自动发现并调用这个 skill。触发关键词:
...
Skill 本身是结构化的 6 步工作流,agent 会逐步引导:
