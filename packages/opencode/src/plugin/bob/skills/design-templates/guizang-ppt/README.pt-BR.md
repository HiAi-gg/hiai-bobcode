...
Uma skill do [Agent Skills](https://agentskills.io/) para gerar **PPTs HTML em arquivo único com swipe horizontal**. O tom visual é "**revista eletrônica × tinta digital**" — como se a *Monocle* tivesse colado código.
...
> Me ajude a instalar a skill `guizang-ppt-skill` do agent. Siga estes passos:
>
> 1. Garanta que o diretório `~/.agent/skills/` existe (crie se não existir)
> 2. Rode `git clone https://github.com/op7418/guizang-ppt-skill.git ~/.agent/skills/magazine-web-ppt`
> 3. Verifique: `ls ~/.agent/skills/magazine-web-ppt/` deve mostrar `SKILL.md`, `assets/` e `references/`
...
Cole esse texto no seu agente e ele faz a instalação automaticamente.

### Forma 2: linha de comando manual

```bash
git clone https://github.com/op7418/guizang-ppt-skill.git ~/.agent/skills/magazine-web-ppt
```

### Como disparar

Depois de instalada, o seu agente descobre e chama a skill automaticamente na conversa. Palavras-chave de gatilho:
...
A skill é um workflow estruturado de 6 passos; o seu agente conduz:
