...
If the `agent-browser` CLI skill is available, verify the generated page:

1. **Start dev server**: Use Bash to start a local server (e.g., `bunx serve site/public`)
2. **Navigate to page**: Run `agent-browser navigate http://localhost:3000/{page}.html`
3. **Capture snapshot**: Run `agent-browser snapshot` to capture the rendered page
4. **Visual comparison**: Compare against the Stitch screenshot (`.stitch/designs/{page}.png`) for fidelity
5. **Stop server**: Terminate the dev server process

> **Note:** This step is optional. If `agent-browser` is not installed, skip to Step 5. Install with `bun add -g agent-browser && agent-browser install`.
