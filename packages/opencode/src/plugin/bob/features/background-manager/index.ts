...
function sortObject(obj: unknown): unknown {
  ...
  return Object.keys(obj as Record<string, unknown>).sort().reduce((acc, key) => { (acc as Record<string, unknown>)[key] = sortObject((obj as Record<string, unknown>)[key]); return acc; }, {} as Record<string, unknown>);
}

function createToolCallSignature(toolName: string, toolInput?: Record<string, unknown>): string {
  if (!toolInput || (typeof toolInput === "object" && Object.keys(toolInput).length === 0)) return toolName;
  return `${toolName}::${JSON.stringify(sortObject(toolInput))}`;
}

function recordToolCallCheck(window: ToolCallWindow | undefined, signature: string, threshold: number): ToolCallWindow {
...
function detectRepetitiveToolUse(window: ToolCallWindow): { triggered: boolean; tool?: string; count?: number } {
...
export class BackgroundManager {
  ...
  constructor(config?: { concurrency_limit?: number; stale_timeout_ms?: number; circuit_breaker?: { enabled?: boolean; max_tool_calls?: number; consecutive_threshold?: number } }) {
    ...
    if (config?.circuit_breaker) {
      this.circuitBreaker = {
        ...this.circuitBreaker,
        ...(config.circuit_breaker.enabled !== undefined && { enabled: config.circuit_breaker.enabled }),
        ...(config.circuit_breaker.max_tool_calls !== undefined && { maxToolCalls: config.circuit_breaker.max_tool_calls }),
        ...(config.circuit_breaker.consecutive_threshold !== undefined && { consecutiveThreshold: config.circuit_breaker.consecutive_threshold }),
      };
    }
  }
  ...
  checkSpawnLimits(parentSessionID: string): boolean {
    ...
    if (count >= this.maxDescendants) {
      console.log(`[hiai-bob] Spawn limit: ${count} descendants for ${parentSessionID}`);
      return false;
    }
  ...
  launch(input: {
    ...
    if (this.runningCount >= this.concurrencyLimit) {
      throw new Error(`Concurrency limit reached (${this.concurrencyLimit}). Wait for existing tasks to complete.`);
    }
  ...
  recordToolCall(taskId: string, toolName: string, toolInput?: Record<string, unknown>) {
    ...
    task.progress.toolCallWindow = recordToolCallCheck(
      task.progress.toolCallWindow, sig, this.circuitBreaker.consecutiveThreshold
    );
  ...
  private async notifyParent(task: BackgroundTask) {
    ...
    try {
      ...
      const notifyPromise = this.client.session.prompt({
        ...
        body: { parts: [{ type: "text", text: message }] } as any,
      });
      await Promise.race([notifyPromise, timeoutPromise]);
    } catch {
      console.log("[hiai-bob] Notification failed or timed out");
    }
  }
