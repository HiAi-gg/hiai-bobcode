...
type TelemetryConfig = HiaiBobConfig["telemetry"];
...
export async function initTelemetry(config: TelemetryConfig | undefined): Promise<void> {
  ...
  try {
    ...
    if (!sdkNode) {
      console.warn("[hiai-bob] Telemetry disabled: @opentelemetry/sdk-node not installed");
      return;
    }

    const serviceName = config.serviceName ?? process.env.OTEL_SERVICE_NAME ?? "hiai-bob";
    ...
    console.warn(`[hiai-bob] Telemetry init failed (continuing without): ${(err as Error).message}`);
  }
}
...
export async function shutdownTelemetry(): Promise<void> {
  ...
  try {
    ...
    console.warn(`[hiai-bob] Telemetry shutdown failed: ${(err as Error).message}`);
  } finally {
    sdk = null;
    initialized = false;
  }
