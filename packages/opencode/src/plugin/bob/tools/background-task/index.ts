...
export const backgroundOutputTool = tool({
  ...
  async execute(args) {
    if (!manager) return { title: "Error", output: "BackgroundManager not initialized." };
    try {
      const task = manager.getTask(args.task_id);
      if (!task) return { title: "Task not found", output: `No task with ID: ${args.task_id}` };
      ...
      return { title: "Error", output: `Failed to get task: ${err instanceof Error ? err.message : String(err)}` };
    }
  },
});
  ...
  async execute(args) {
    ...
    try {
      ...
      return success
        ...
        : { title: "Task not found", output: `No task with ID: ${args.task_id}` };
    } catch (err) {
      return { title: "Error", output: `Failed to cancel task: ${err instanceof Error ? err.message : String(err)}` };
    }
  },
});