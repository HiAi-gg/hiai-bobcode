...
// DISABLED for hiai-bob: no auth connection to xiaomimimo.com
...
// DISABLED for hiai-bob — auth connection to xiaomimimo platform is not used.
// The provider config hooks and auth methods below are stubbed out.
export async function MimoAuthPlugin(_input: PluginInput): Promise<Hooks> {
  return {
    config: async (input) => {
      ...
      xiaomi.name ??= "Bob"
      xiaomi.api ??= "https://api.hiai.dev/v1"
    },
    auth: {
      provider: "xiaomi",
