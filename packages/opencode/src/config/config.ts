          ...
          const dep = yield* npmSvc
            ...
            .pipe(
              ...
              Effect.tap((exit) =>
                Exit.isFailure(exit)
                  ? Effect.sync(() => {
                      // Non-fatal: this best-effort install of @mimo-ai/plugin only
                      // matters for EXTERNAL plugins, and the exact prod build version
                      // isn't published to npm (it's a bundled workspace package), so
                      // it always "fails" harmlessly. Keep it at debug to avoid noise.
