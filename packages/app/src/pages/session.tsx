...
export default function Page(props: { sessionID?: string; mode?: "full" | "cell" }) {
  ...
  const { params, sessionKey, tabs, view } = useSessionLayout(props.sessionID)
  ...
  createEffect(
    on(
      ...
      (id, prev) => {
        if (!id) return
        if (prev) return
        // Missing/slow session must not crash the app: a bare reject here is an
        // unhandled rejection that trips the root ErrorBoundary (whole tree
        // unmounts → solid-dnd teardown spam). Swallow it; the UI shows the
        // empty/loading session state instead.
  ...
  const [sessionSync] = createResource(
    ...
    ([directory, id]) => {
      ...
      // Resolve to undefined on failure (e.g. a stale/missing session 404s).
      // This resource is read in JSX (`{sessionSync() ?? ""}`); a rejected
      // resource re-throws on read and trips the root ErrorBoundary, taking the
      // whole app down. Degrade to the empty-session UI instead.
      return sync.session.sync(id).catch(() => undefined)
    },
  )
  ...
  return (
    <div class="relative bg-background-base size-full overflow-hidden flex flex-col">
      {sessionSync() ?? ""}
      {/* Header portals its controls into the shared top bar. In a grid only the
          ACTIVE cell renders in "full" mode, so cell-mode cells skip the header
          and the top controls aren't duplicated per cell. */}
