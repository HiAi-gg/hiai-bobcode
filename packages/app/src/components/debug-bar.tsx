...
// Collapsed state persists across reloads — dev-only, so plain localStorage is
// enough (no need for the async Persist layer). Collapsed acts as the "off"
// switch: the panel folds to a small corner pill you can expand on demand.
const COLLAPSE_KEY = "mimo.debugBar.collapsed"
const readCollapsed = () => {
  try {
  ...
  return (
    <Show
      ...
      fallback={
        <button
          type="button"
          aria-label={`${language.t("debugBar.ariaLabel")} — expand`}
          title={`${language.t("debugBar.ariaLabel")} — expand`}
          onClick={toggle}
          class="pointer-events-auto fixed bottom-3 right-3 z-50 flex h-7 items-center gap-1 rounded-full border border-border-base bg-surface-raised-stronger-non-alpha px-2.5 font-mono text-[10px] font-black uppercase tracking-[0.04em] text-text-strong opacity-70 shadow-[var(--shadow-lg-border-base)] hover:opacity-100 sm:bottom-4 sm:right-4"
        >
    ...
    >
      ...
      >
        <button
          type="button"
          aria-label={`${language.t("debugBar.ariaLabel")} — collapse`}
          title={`${language.t("debugBar.ariaLabel")} — collapse`}
          onClick={toggle}
          class="absolute right-1 top-1 z-10 flex size-4 items-center justify-center rounded text-[11px] leading-none text-text-strong opacity-50 hover:opacity-100"
        >
          <span aria-hidden="true">✕</span>
        </button>
