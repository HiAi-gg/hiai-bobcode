...
/**
 * Branded workspace identifier. Aliasing `string` keeps call sites ergonomic
 * while preventing accidental cross-domain confusion (e.g. handing a sessionID
 * to a method that wants a workspaceID). Mirrors the TUI's branding.
 */
...
/**
 * Per-workspace SDK client lifecycle owner. One pool is created per app
 * session and exposed via `WorkspaceClientsProvider`; cells call
 * `get(workspaceID)` to acquire a client and `release(workspaceID)` when
 * they no longer need it. The pool refcounts so the underlying client
 * cache is only flushed after the last reference for a workspace drops —
 * multiple cells viewing the same workspace therefore share a single
 * client (and a single event stream).
 *
 * Lifecycle:
 ...
 *   4. `destroy()` — flush every cached client. Called on app shutdown.
 */
export class WorkspaceClientPool {
   ...
   * reaches zero the cached client is flushed via `cleanup` so we don't
   * leak event listeners or SSE streams across long-running sessions.
   */
  ...
  /**
   * Force-evict a workspace regardless of refcount. Returns the entries
   * that were flushed so callers can null-check their stored references.
   */
...
/**
 * Context API exposed to consumers. Wraps the pool with convenience
...
export type WorkspaceClientsContext = {
   ...
   * `get()` + `release()` under the hood so refcounts track cell
   * lifecycles correctly across Solid re-renders.
   */
...
export const { use: useWorkspaceClients, provider: WorkspaceClientsProvider } = createSimpleContext({
  ...
  init: (): WorkspaceClientsContext => {
    ...
    const pool = new WorkspaceClientPool({
      create: (id) => globalSDK.clientForWorkspace(id),
      cleanup: (id) => globalSDK.cleanupWorkspaceClient(id),
    })
...
export function createWorkspaceClientAccessor(input: {
  workspaceID: Accessor<WorkspaceID | undefined>
}): Accessor<OpencodeClient | undefined> {
