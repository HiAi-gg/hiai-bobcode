...
<p align="center">
  <a href="https://hiai.dev/zh/hiai-bob">官网</a> | <a href="https://hiai.dev/zh/blog/bob-code-long-horizon">博客</a>
</p>

---

hiai-bob 是一个终端原生的 AI 编程助手。它能读写代码、执行命令、管理 Git，通过持久化记忆系统，在多次会话间保持对你项目的深度理解，并自我进化。

内置 Bob Auto 限时免费通道——零配置即可开始使用。也支持接入各家主流 LLM 厂商 API。
...
基于 TenVAD 和 Bob ASR 的实时流式语音输入。通过 `/voice` 激活，按停顿分片转写，文本逐段追加到输入框。仅对 Bob 登录用户可用。
...
通过项目目录下的 `.mimocode/mimocode.json`（或全局 `~/.config/hiai-bob/mimocode.json`）配置。主要选项包括：
...
hiai-bob 基于 [OpenCode](https://github.com/anomalyco/opencode) fork 构建，保留其全部核心能力（多 Provider、TUI、LSP、MCP、插件），并在此基础上构建了持久化记忆、智能上下文管理、子智能体编排、目标驱动的自主循环、Compose 工作流，以及通过 dream/distill 实现的自我进化。
...
使用 hiai-bob 还需遵守[使用限制](./USE_RESTRICTIONS.md)。
使用 Bob 托管服务须遵守 [Bob 服务条款](https://platform.hiai-opencode.com/docs/terms/user-agreement)。
使用 Bob 名称、标志和商标须遵守 Bob 商标政策。
