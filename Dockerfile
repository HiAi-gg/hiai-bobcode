...
RUN bun -e "const{readFileSync,writeFileSync}=require('fs');const p=JSON.parse(readFileSync('package.json','utf8'));p.workspaces.packages=['packages/app','packages/opencode','packages/plugin','packages/script','packages/ui','packages/shared','packages/sdk/js'];writeFileSync('package.json',JSON.stringify(p,null,2))"
