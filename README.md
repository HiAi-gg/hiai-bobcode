...
<p align="center">
  <a href="https://hiai.dev/en/hiai-bob">Website</a> | <a href="https://hiai.dev/en/blog/bob-code-long-horizon">Blog</a>
</p>

---

hiai-bob is a terminal-native AI coding assistant. It can read and write code, run commands, manage Git, and use a persistent memory system to keep a deep understanding of your project across sessions while continuously improving itself.

Bob Auto is built in as a free-for-limited-time channel, so you can start with zero configuration. hiai-bob also supports connecting to any mainstream LLM provider API.
...
```bash
# One-line install
curl -fsSL https://hiai.dev/install | bash
...
- **Bob Auto (free for a limited time)** — anonymous channel, zero configuration
- ** Bob Platform** — OAuth login
...
Real-time streaming voice input powered by TenVAD and Bob ASR. Activate with `/voice`, then speak — audio is segmented by pauses and transcribed incrementally into the input. Available for Bob logged-in users.
...
hiai-bob is configured via `.mimocode/mimocode.json` in the project directory (or `~/.config/hiai-bob/mimocode.json` globally). Key options include:
...
Use of hiai-bob is also subject to the [Use Restrictions](./USE_RESTRICTIONS.md).
Use of  Bob-hosted services is subject to the [Bob Terms of Service](https://platform.hiai-opencode.com/docs/terms/user-agreement).
Use of the Bob name, logo, and trademarks is subject to the Bob Trademark Policy.
