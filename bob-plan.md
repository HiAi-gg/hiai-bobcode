...
> ⚠️ **HISTORICAL — SUPERSEDED by §F below.** This section describes the OLD external-plugin architecture (pre-fork). The current fork uses `@mimo-ai/plugin` (workspace), oxlint + Prettier, and different file paths. See §F for the active development plan.
...
> - **`bob-todo.md §R` = feature spec** — _what_ to build (agents, autonomy, Critic gate, parallelism);
...
describe("decide", () => {
  ...
  test("incomplete todos at cap -> stop(cap)", () => {
    expect(decide({ ...base, hasIncompleteTodos: true, autoContinues: 25 })).toEqual({ kind: "stop", reason: "cap" })
  })
  ...
  test("todos done, require_critic=false -> stop(done) even with changes", () => {
    expect(decide({ ...base, requireCritic: false, changedFiles: ["a.ts"], currentFingerprint: "x" })).toEqual({
      kind: "stop",
      reason: "done",
    })
  })
...
const CONTINUE_PROMPT = "Continue with the remaining TODO items until all are complete. Do not stop early."
const FIX_PROMPT = "The Critic REJECTED the current changes. Address every point it raised, then continue."
const REVIEW_PROMPT =
...
export function createCompletionController(config: HiaiBobConfig): HookSet {
  ...
  return {
    ...
    dispose: async () => {
      /* state is in-memory; nothing persistent */
    },
  }
